...
PRD1C3A activates only `api`, `worker`, `delivery`, `web`, and then `gateway`, using immutable image IDs recorded by PRD1C2. When the verified target Compose source declares `storage-init`, rollout pins it to the API image ID and runs the oneshot (`docker compose run --rm --no-deps`) under the rollout lock **before** application mutation. Rollout never relies on Compose `depends_on` (`--no-deps`). A failed initializer stops rollout before `activating_app`. Older prepared releases without `storage-init` remain valid; rollback/recovery to those releases skips the oneshot. PostgreSQL is neither recreated nor migrated by `rollout`. Health does not require the oneshot to remain running. The read-only database gate requires live Alembic heads to match saved `current.database.heads`, then allows the target application only when its source heads equal those DB heads **or** its revision is listed in the explicit compatibility envelope (see [chapter 28](28-dual-application-database-state.md)). Bootstrap still requires live heads to equal the target release source heads.
