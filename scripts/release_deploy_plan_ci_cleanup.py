#!/usr/bin/env python3
"""CI fallback cleanup for PRD1C3B1 deploy-plan integration (exact project only)."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.project_file.is_file():
        ...
        return 0
    try:
        project = load_deploy_plan_project_file(args.project_file)
    ...
    with tempfile.TemporaryDirectory(prefix="fetchnow-deploy-plan-cleanup-") as tmp:
        ...
        env_file.write_text(
            "\n".join(
                [
                    ...
                    "FETCHNOW_DEPLOY_ROOT=/tmp/fetchnow-deploy-plan-cleanup-deploy",
                    "FETCHNOW_BACKUP_ROOT=/tmp/fetchnow-deploy-plan-cleanup-backup",
                ]
            )
            + "\n",
            encoding="utf-8",
