...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        run(["git", "add", "backend/migrations/versions/0002_expand.py", str(contract_path)], cwd=clone)
        ...
        if plan3["included_revisions"] != ["0002_expand"]:
        ...
        # Divergent target: DB/current at 0002_expand, target graph head is sibling 0003_diverge only.
        run(["git", "checkout", "--detach", migrate_rev], cwd=clone)
        run(compose + ["run", "--rm", "api", "alembic", "upgrade", "head"], cwd=clone)
        seed_current_state(
            deploy_root, release_dir(deploy_root, migrate_rev), migrate_rev
        )
        (mig_versions / "0002_expand.py").unlink()
        (mig_versions / "0003_diverge.py").write_text(
            'revision = "0003_diverge"\ndown_revision = "0001_baseline"\n',
            encoding="utf-8",
        )
                    ...
                    "transitions": [
                        {
                            ...
                            "to_heads": ["0003_diverge"],
                            "included_revisions": ["0003_diverge"],
                            "execution_mode": "online_expand",
                            "online_with_previous_application": True,
                            "previous_application_rollback_compatible": True,
                            "data_loss": "none",
        ...
        run(
            ["git", "add", str(mig_versions / "0003_diverge.py"), str(diverge_contract)],
            cwd=clone,
        )
        run(["git", "rm", str(mig_versions / "0002_expand.py")], cwd=clone)
        ...
        # Downgrade rejection: current release + DB at 0002_expand, target v2 baseline release.
        run(["git", "checkout", "--detach", migrate_rev], cwd=clone)
        run(compose + ["build", "api"], cwd=clone)
        run(compose + ["run", "--rm", "api", "alembic", "upgrade", "head"], cwd=clone)
        seed_current_state(
