...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        run(["git", "add", "backend/migrations/versions/0003_expand.py", str(contract_path)], cwd=clone)
        ...
        if plan3["included_revisions"] != ["0003_expand"]:
        ...
        # Divergent target: DB/current at 0003_expand, target graph head is sibling 0003_diverge only.
        run(["git", "checkout", "--detach", migrate_rev], cwd=clone)
        run(compose + ["run", "--rm", "api", "alembic", "upgrade", "head"], cwd=clone)
        seed_current_state(
            deploy_root, release_dir(deploy_root, migrate_rev), migrate_rev
        )
        (mig_versions / "0003_expand.py").unlink()
        ...
        run(["git", "rm", str(mig_versions / "0003_expand.py")], cwd=clone)
        ...
        # Downgrade rejection: current release + DB at 0003_expand, target v2 baseline release.
        run(["git", "checkout", "--detach", migrate_rev], cwd=clone)
        run(compose + ["build", "api"], cwd=clone)
        run(compose + ["run", "--rm", "api", "alembic", "upgrade", "head"], cwd=clone)
        seed_current_state(
