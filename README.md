...
- Bounded muxing: base Free worker capability using stream-copy of compatible video-only + audio-only streams ([ADR 0013](docs/adr/0013-bounded-media-muxing.md)). Direct files remain preferred for the same quality. `MEDIA_MUXING_ENABLED` defaults false pending controlled activation; ffmpeg/ffprobe paths never reach API, delivery, or web.
