...
function snapshot(overrides: Partial<FlowSnapshot> = {}): FlowSnapshot {
  ...
  return {
    ...new MediaFlowController().snapshot(),
    phase: "inspected",
    premiumState: "free",
    formats,
    result: {
...
beforeEach(() => {
  document.body.innerHTML =
    "<fieldset data-flow-quality><div data-flow-formats></div></fieldset><p data-flow-selection-summary></p><button data-flow-download></button>";
});
...
describe("A4 capability selector", () => {
  it("keeps category identity, one global radio selection and compact real metadata", () => {
    ...
    expect(
      [...document.querySelectorAll<HTMLElement>("[data-category-panel]")].map(
        (p) => p.dataset.categoryPanel,
      ),
    ).toEqual(["normal_video", "video_only", "audio_only"]);
    ...
    expect(panel("audio_only").querySelector(".format-label")?.textContent).toBe(
      "128 кбит/с",
    );
    expect(panel("audio_only").querySelector(".format-detail")?.textContent).toContain(
      "M4A · ≈",
    );
    expect(
      panel("audio_only").querySelector("input")?.getAttribute("aria-label"),
    ).toContain("Без изображения");
  ...
  it("preserves radio focus through selection rendering and exposes just one selected option", () => {
    ...
    expect((document.activeElement as HTMLInputElement).value).toBe(
      audio.formatOptionId,
    );
    expect(document.querySelectorAll("input:checked")).toHaveLength(1);
    expect(
      document.querySelector("[data-flow-selection-summary]")?.textContent,
    ).toContain("Только аудио · 128 кбит/с · M4A");
  });

  it("starts on the valid selected capability and resets presentation for a new inspection", () => {
    renderFlow(
      document,
      snapshot({ premiumState: "active", selectedFormatId: video.formatOptionId }),
    );
  ...
  // An unresolved Premium lookup reads as Free: same locked copy, no narration
  // of a background check the visitor never asked for.
  it.each([
    ["free", "Доступно с Premium", true],
    ["loading", "Доступно с Premium", true],
    ["error", "Доступно с Premium", true],
    ["active", "", false],
  ] as const)(
    "renders %s without misrepresenting entitlement",
    (premiumState, help, disabled) => {
      renderFlow(document, snapshot({ premiumState }));
      expect(
        panel("audio_only").querySelector<HTMLInputElement>("input")?.disabled,
      ).toBe(disabled);
      expect(panel("audio_only").querySelector(".category-help")?.textContent).toBe(
        help,
      );
      expect(
        panel("normal_video").querySelector<HTMLInputElement>("input")?.disabled,
      ).toBe(false);
    },
  );
  ...
  it("handles unknown audio bitrate/size without fake resolution, size or MP3", () => {
    renderFlow(
      document,
      snapshot({ formats: [{ ...audio, bitrateKbps: null, approxBytes: null }] }),
    );
    expect(panel("audio_only").querySelector(".format-label")?.textContent).toBe(
      "Аудиодорожка",
    );
    expect(panel("audio_only").querySelector(".format-detail")?.textContent).toBe(
      "M4A",
    );
  ...
  it("shows a single auto-selected normal option and technical empty states without paywalls", () => {
    ...
    expect(panel("audio_only").textContent).toContain(
      "Для этой ссылки нет доступных вариантов аудио",
    );
  ...
  it("locks the common action while busy and removes selector during preparation", () => {
    ...
    expect(
      document.querySelector<HTMLElement>("[data-flow-selection-summary]")?.hidden,
    ).toBe(true);
  });
});
...
describe("A4 standalone display ordering", () => {
  it("orders known audio bitrate before unknown and preserves a selected duplicate deterministically", () => {
    const high = {
      ...audio,
      formatOptionId: "fmt_dddddddddddddddddddddddddddd",
      bitrateKbps: 192,
    };
    const unknown = {
      ...audio,
      formatOptionId: "fmt_eeeeeeeeeeeeeeeeeeeeeeeeeeee",
      bitrateKbps: null,
    };
    const duplicate = {
      ...audio,
      formatOptionId: "fmt_ffffffffffffffffffffffffffff",
      container: "webm",
    };
    ...
    expect(
      groupStandaloneOptions([...formats].reverse(), true, duplicate.formatOptionId),
    ).toEqual(grouped);
  });
  it("does not use video bitrate as resolution ordering and has stable ties", () => {
    ...
    expect(groupStandaloneOptions([...formats].reverse(), false, null)).toEqual(
      grouped,
    );
  });
});
