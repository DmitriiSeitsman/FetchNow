...
describe("quality grouping", () => {
  it("uses human quality tiers while preserving technical resolution", () => {
    expect(qualityDisplayLabel(format({ height: 1080 }))).toBe("Высокое (1080p)");
    expect(qualityDisplayLabel(format({ height: 720 }))).toBe("Высокое (720p)");
    expect(qualityDisplayLabel(format({ height: 576 }))).toBe("Среднее (576p)");
    expect(qualityDisplayLabel(format({ height: 480 }))).toBe("Среднее (480p)");
    expect(qualityDisplayLabel(format({ height: 360 }))).toBe("Низкое (360p)");
    expect(
      qualityDisplayLabel(format({ height: null, qualityLabel: "source" })),
    ).toBe("Качество (source)");
  });
  ...
  it("keeps one representative for two eligible 720p formats and prefers MP4", () => {
    ...
    expect(options[0].label).toBe("Высокое (720p)");
  ...
  it("groups formats without a height by their strictly normalized label", () => {
    ...
    expect(options[1].label).toBe("Качество (source ??)");
  ...
  it("does not merge a malformed label group with a numeric height group", () => {
    ...
    expect(options[1].label).toBe("Высокое (720p)");
  });
